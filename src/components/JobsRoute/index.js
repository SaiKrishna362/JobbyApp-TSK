...
import FilterGroup from '../FilterGroupRoute'
...
class Jobs extends Component {
  ...
  getJobs = async () => {
    ...
    if (employeeType.length === 0) {
      val = 'FULLTIME,PARTTIME,FREELANCE,INTERNSHIP'
    } else {
      val = employeeType.join(',')
    }
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${val}&minimum_package=${minimumSalary}&search=${searchInput}`
  ...
  changeEmployeeList = type => {
    ...
    const app = employeeType.includes(type)

    if (app) {
      this.setState(
        prevState => ({
          employeeType: prevState.employeeType.filter(
            eachList => eachList !== type,
          ),
        }),
        this.getJobs,
      )
